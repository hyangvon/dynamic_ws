...
用法：
    # 单个CSV目录
    python3 analyze_vi_results.py --path src/w10_force_sim/csv/q0p2_dt0p01_T10_a0p4_b0p04/ctsvi_ad/
    
    # 多个子文件夹（会自动处理所有子文件夹）
    python3 analyze_vi_results.py --path src/w10_force_sim/csv/q0p2_dt0p01_T10_a0p4_b0p04/
    
    # 显示图表
    python3 analyze_vi_results.py --path src/w10_force_sim/csv/q0p2_dt0p01_T10_a0p4_b0p04/ --plot
    
    # 自定义配置文件
    python3 analyze_vi_results.py --path src/w10_force_sim/csv/q0p2_dt0p01_T10_a0p4_b0p04/ --config config/plot_config.yaml
    
    # 自定义保存路径
    python3 analyze_vi_results.py --path src/w10_force_sim/csv/q0p2_dt0p01_T10_a0p4_b0p04/ --save-plot /tmp/output.png

配置文件：
    - 默认配置文件位置: src/w10_force_sim/config/plot_config.yaml
...
def load_plot_config(config_path=None):
    ...
    if config_path is None:
        # 尝试从默认位置查找配置文件
        default_paths = [
            Path('src/w10_force_sim/config/plot_config.yaml'),
...
def _save_plot(fig, csv_dir, custom_save_path, subfolder_name=None, parent_name=None):
    ...
    if custom_save_path:
        ...
        try:
            ...
            parts = csv_dir.parts
            if 'w10_force_sim' in parts:
                w10_sim_idx = parts.index('w10_force_sim')
