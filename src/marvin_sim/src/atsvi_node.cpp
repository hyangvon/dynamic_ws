...
int main(int argc, char** argv)
{
    ...
    double avg_time = 0.0;  // 外层作用域，CSV 写入时需要

    { // AD 作用域：model_ad/data_ad 在此层析构，避免 CppAD thread_alloc 与节点析构冲突
    ...
    avg_time = !runtimes.empty() ? std::accumulate(runtimes.begin(), runtimes.end(), 0.0) / runtimes.size() : 0.0;
    RCLCPP_INFO(node->get_logger(), "Simulation finished, wall time: %f s, Average step time: %f ms", total_elapsed, avg_time*1e3);
    } // end AD scope: model_ad 和 data_ad 在此正确析构，避免 CppAD heap 捅崩
    ...
    // 显式 shutdown rclcpp 并释放 node，让 rclcpp 在 Pinocchio/CppAD 全局析构前先清理
    // 否则 CppAD thread_alloc 大量 mmap 块与 rclcpp 析构顺序冲突，导致 munmap_chunk
    rclcpp::shutdown();
    node.reset();

    // 显式释放大型容器，确保堆在 Pinocchio Data/Model 析构前处于干净状态
    q_history.clear();         q_history.shrink_to_fit();
    momentum_history.clear();  momentum_history.shrink_to_fit();
    force_torque_history.clear(); force_torque_history.shrink_to_fit();
    centroidal_lin_momentum_history.clear(); centroidal_lin_momentum_history.shrink_to_fit();
    ee_history.clear();        ee_history.shrink_to_fit();
    energy_history.clear();    energy_history.shrink_to_fit();
    delta_energy_history.clear();
    time_history.clear();
    h_history.clear();
    runtimes.clear();
