...
State project_to_energy(const Model &model, Data &data, const State &candidate, double E_ref)
{
    double U_new = potential_energy(model, data, candidate.q);
    double T_new = kinetic_energy(model, data, candidate.q, candidate.v);
    double T_target = E_ref - U_new;

    if (T_target <= 0.0) {
        // The candidate state is already beyond the available energy level.
        // Fall back to zero kinetic energy to avoid taking imaginary scaling.
        State projected = candidate;
        projected.v = Vec::Zero(candidate.v.size());
        return projected;
    }
    ...
    double alpha = std::sqrt(std::max(0.0, T_target / T_new));
    if (!std::isfinite(alpha)) {
        return candidate;
    }

    State projected = candidate;
    projected.v = alpha * candidate.v;
    return projected;
}
...
int main(int argc, char** argv)
{
    ...
    for (int step = 0; step < n_steps && t_cur < duration - 1e-12; ++step)
    {
        ...
        state_next = project_to_energy(model, data, state_next, E_init);
        state = state_next;
    ...
    std::string csv_dir = std::string("src/vi_2p/csv/") + params_label + std::string("/rk4_proj/");
    ...
    try {
        if (!pin_runtimes.empty()) {
            ...
            pin_time_file << (avg_pin * 1000.0) << std::endl;
            pin_time_file.close();
